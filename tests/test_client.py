...
class BaseFuncTestCase(ClientTestCase):
    ...
    def test_make_auth_header(self):
        self.assertEqual(
            first=raw.default_client.make_auth_header(),
    ...
    def test_access_token_has_expired(self):
        ...
        test_cases = {
            "fresh": (datetime.timedelta(days=30), False),
        ...
        for testcase in test_cases.items():
            client.access_token = jwt.encode(
                payload={
                    "exp": (
                        datetime.datetime.now() + testcase[-1][0]
                    ).timestamp()
                },
                key="secretkey",
            )
            ...
            self.assertEqual(
                first=client.access_token_has_expired,
                second=testcase[-1][-1],
        ...
        self.assertEqual(
            first=client.access_token_has_expired, second=False, msg=""
        )
    ...
    def test_automatic_token_refresh(self):
        ...
        expired_access_token = encode(
            (
                datetime.datetime.now() + datetime.timedelta(days=-30)
            ).timestamp()
        )
        fresh_access_token = encode(
        ...
        with requests_mock.Mocker() as mock:
            ...
            client.session.get(
                raw.get_full_url(path="test", client=client),
    ...
    def test_init_refresh_token(self):
        access_token = jwt.encode(
            payload={"exp": datetime.datetime.now()}, key="secretkey"
        )
